// Skema + seed data lokal Pesanin (SQLite / better-sqlite3).
...
const DEMO_USERS = [
  { id: 'u_merchant', full_name: 'Bu Sri', email: 'merchant@demo.test', role: 'merchant', avatar: '🍔', idr_balance: 1240000, matic_balance: 182.4 },
  { id: 'u_buyer', full_name: 'Rina Pratiwi', email: 'buyer@demo.test', role: 'buyer', avatar: '👩', address: 'Kos Putri Melati, Jl. Cempaka Sari No.7', idr_balance: 84500, matic_balance: 12.4 },
  { id: 'u_driver', full_name: 'Andi Saputra', email: 'driver@demo.test', role: 'driver', avatar: '🏍️', idr_balance: 327000, matic_balance: 48.1 },
];
...
const SEED_ORDERS = [
  { id: 'a1b2c3d4e5f6', merchant_id: 'm1', driver_id: null, subtotal: 27000, delivery_fee: 6000, gas_fee: 1500, total: 34500, delivery_address: 'Kos Putri Melati, Jl. Cempaka Sari', status: 'paid', created_at: minsAgo(4), items: [{ name: 'Nasi Rames Komplit', qty: 1, price: 13000 }, { name: 'Soto Ayam Semarang', qty: 1, price: 14000 }] },
  { id: 'b2c3d4e5f6a7', merchant_id: 'm1', driver_id: null, subtotal: 18000, delivery_fee: 5000, gas_fee: 1500, total: 24500, delivery_address: 'Gedung H FMIPA Unnes', status: 'ready_for_pickup', created_at: minsAgo(11), items: [{ name: 'Soto Ayam Semarang', qty: 1, price: 14000 }, { name: 'Es Teh Manis', qty: 1, price: 4000 }] },
  { id: 'c3d4e5f6a7b8', merchant_id: 'm1', driver_id: null, subtotal: 41000, delivery_fee: 8000, gas_fee: 1500, total: 50500, delivery_address: 'Asrama Mahasiswa, Jl. Kalimasada', status: 'accepted_merchant', created_at: minsAgo(18), items: [{ name: 'Nasi Rames Komplit', qty: 3, price: 13000 }] },
  { id: 'd4e5f6a7b8c9', merchant_id: 'm1', driver_id: 'u_driver', subtotal: 22000, delivery_fee: 6000, gas_fee: 1500, total: 29500, delivery_address: 'Kontrakan Biru, Jl. Patemon Raya', status: 'picked_up', created_at: minsAgo(26), items: [{ name: 'Pecel Sayur', qty: 2, price: 10000 }] },
  { id: 'e5f6a7b8c9d0', merchant_id: 'm1', driver_id: null, subtotal: 15000, delivery_fee: 5000, gas_fee: 1500, total: 21500, delivery_address: 'Masjid Ulul Albab Unnes', status: 'paid', created_at: minsAgo(33), items: [{ name: 'Soto Ayam Semarang', qty: 1, price: 14000 }] },
];
...
export function seed(db) {
  ...
  const insUser = db.prepare(
    `INSERT INTO users (id, full_name, email, phone, password_hash, role, avatar, address, idr_balance, matic_balance)
     VALUES (@id, @full_name, @email, '', @password_hash, @role, @avatar, @address, @idr_balance, @matic_balance)`
  );
  const hash = bcrypt.hashSync('demo123', 10);
  DEMO_USERS.forEach((u) =>
    insUser.run({ address: '', matic_balance: 0, idr_balance: 0, ...u, password_hash: hash })
  );
  ...
  const insOrder = db.prepare(
    `INSERT INTO orders (id, buyer_id, merchant_id, driver_id, subtotal, delivery_fee, gas_fee, total, status, delivery_address, tx_hash, created_at)
     VALUES (@id, @buyer_id, @merchant_id, @driver_id, @subtotal, @delivery_fee, @gas_fee, @total, @status, @delivery_address, @tx_hash, @created_at)`
  );
